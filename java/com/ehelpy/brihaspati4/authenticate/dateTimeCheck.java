...
public class dateTimeCheck 
{
	...
	public static String c_date_time;
		...
		try
		{
			String sdate=getServerHttpDate(serverUrl);
			if (sdate==null)
	        	{	
				date1=sdf2.parse(getLastLogoutTime());
	    			System.out.println("last logout  Date & Time is :   " +date1 );
			}else{	
		...
		if (sdf2.format(date1).compareTo(getCurrentDateTime()) <= 0 ) {		
			// if server date is less than or equal to system date then system clock ok
			...
			// if server date is not less than or equal to system date then system clock incorrect
		 	System.out.println("Server Date & Time is :   " +sdf2.format(date1) );
		    	debug_level.debug(3,"Your System Date & Time is :   " + getCurrentDateTime());
	    		//debug_level.debug(0,"Last logout date & time of your system is:   " + getLastLogoutTime());
	    	...
	    	return flag; 
		// returns false to intimate user to correct system date and time and true to log on the network if system clock correct
	}
	...
	private static String getServerHttpDate(String serverUrl) throws IOException {
		try{
	    	...
	    	for (Map.Entry<String, List<String>> entry : httpHeaders.entrySet()) {
				...
				// retrieving server date and time and returning to main function
	      		}	
	    	}
		}
		catch(Exception ex){
			System.out.println("The error is "+ex.toString());
	    		return null;											
		// returns null if server not online
		}
	    		return null;											
	}
